...
class Wampy {
    ...
    async _onInvocationMessage ([, requestId, topic, details, argsList, argsDict]) {
        const self = this;
        const handleInvocationError = ({ errorURI, details, argsList, argsDict }) => {
            self._send([
                WAMP_MSG_SPEC.ERROR,
                WAMP_MSG_SPEC.INVOCATION,
                requestId,
                details || {},
                errorURI || 'wamp.error.invocation_exception',
                argsList || [],
                ...(argsDict ? [argsDict] : [])
            ]);
        };
        ...
        if (!this._rpcRegs[topic]) {
            ...
            return handleInvocationError({ errorURI: 'wamp.error.no_such_procedure' });
        }
